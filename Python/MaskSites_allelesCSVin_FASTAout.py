...
#example command "python MaskSites_allelesCSVin_FASTAout.py -a test_alleles.csv -b test_alleles_blocks.txt"
...
parser.add_argument('-a', '--alignment_file', help = "Name of input 'x_alleles.csv' SNP alignment file.", required = True)
...
#Here we will set a global variable (type=dictionary) containing keys as taxon names, and
#values as a list containing block-position tuples
dict_of_blocks = {}

#Here we will write the block positions from blocks_file to "dict_of_blocks" global above
...
def alignment_mask(alignment_file, blocks, character):
	df = pd.read_csv(alignment_file)
	colnames = list(df.columns.values)
	#rownames_header=names[0].  This should either be "SNP' or "Pos" in the "x_alleles.csv".
	with open(os.path.splitext(alignment_file)[0]+'_RecombMasked.fasta', 'w') as output_handle:
		print '\nMasking SNPs in "'+alignment_file+'" with "'+character+'".'
